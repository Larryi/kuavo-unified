...
NORM_FILE="${RUN_DIR}/norm_stats/task1_345.json"
FINAL_HF_DIR="${RUN_DIR}/final_hf_ckpt"
LOG_DIR="${WORK_ROOT}/logs/${RUN_ID}"
PIPELINE_LOG="${LOG_DIR}/pipeline.log"
TRAIN_LOG="${LOG_DIR}/train.log"
TRAIN_CONFIG="${CODE_DIR}/configs/policy/lingbot/task1_345_full.yaml"
...
stop_vast_instance() {
    ...
    if ! command -v vastai >/dev/null 2>&1; then
        notify "TASK1 LingBot自动停机失败" $'原因：vastai CLI不可用\nRun：'"${RUN_ID}"$'\n状态：'"${status}"
        return 0
    fi
    if ! instance_id="$(resolve_vast_instance_id)"; then
        notify "TASK1 LingBot自动停机失败" $'原因：无法识别Vast实例ID\nRun：'"${RUN_ID}"$'\n请设置VAST_INSTANCE_ID'
        return 0
    fi

    notify "TASK1 LingBot流水线结束，准备停机" $'Run：'"${RUN_ID}"$'\n状态：'"${status}"$'\n实例：'"${instance_id}"$'\n等待：'"${STOP_DELAY_SECONDS}"'秒'
    sleep "${STOP_DELAY_SECONDS}"
    if ! vastai stop instance "${instance_id}"; then
        notify "TASK1 LingBot自动停机失败" $'Run：'"${RUN_ID}"$'\n实例：'"${instance_id}"$'\n请手动停止实例'
    fi
}
...
on_exit() {
    ...
    if (( rc != 0 )) && (( FAIL_NOTIFIED == 0 )); then
        notify "TASK1 LingBot流水线异常" $'阶段：'"${PIPELINE_PHASE}"$'\n退出码：'"${rc}"$'\nRun：'"${RUN_ID}"$'\n日志：\n'"$(tail -n 35 "${PIPELINE_LOG}" 2>/dev/null || true)"
    fi
...
assert info["total_episodes"] == 345, info["total_episodes"]
assert info["total_frames"] == 81142, info["total_frames"]
assert info["fps"] == 10, info["fps"]
assert info["features"]["observation.state"]["shape"] == [8]
assert info["features"]["action"]["shape"] == [8]
assert set(k for k in info["features"] if "images." in k) == {
    "observation.images.head_cam_h", "observation.images.wrist_cam_r"
}
...
(
    while sleep "${HEARTBEAT_SECONDS}"; do
        ...
        notify "TASK1 LingBot训练心跳" $'Run：'"${RUN_ID}"$'\n'"${loss_line:-no loss yet}"$'\n磁盘：'"${disk}"
    done
) &
HEARTBEAT_PID=$!
...
if (( train_rc != 0 )); then
    FAIL_NOTIFIED=1
    notify "TASK1 LingBot训练失败" $'Run：'"${RUN_ID}"$'\n退出码：'"${train_rc}"$'\n日志：\n'"$(tail -n 35 "${TRAIN_LOG}" 2>/dev/null || true)"
    exit "${train_rc}"
fi
...
notify "TASK1 LingBot训练与上传完成" $'Run：'"${RUN_ID}"$'\n模型：https://huggingface.co/'"${MODEL_REPO}"$'\n最新DCP：'"${checkpoint}"
