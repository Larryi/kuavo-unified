...
- `kuavo_v2` maps a single-arm dataset as 7 relative joint positions plus one absolute gripper value.
...
LINGBOT_V2_ROOT=/home/larry/lingbot-vla-v2 \
  ...
  assets/norm_stats/kuavo_v2_meanstd.json
```

The committed Task1 config uses per-joint MeanStd after applying relQpos. For Task2, copy the norm config, use `data_name=kuavo_v2_bimanual`, include all three cameras, and write a separate bimanual norm file.
...
- `configs/robot_configs/kuavo_v2.yaml` or `kuavo_v2_bimanual.yaml`;
